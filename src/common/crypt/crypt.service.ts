...
export class CryptService {
  ...
  static encryptConfirm(id: number): string {
    const data = `${id}`;
  ...
  static decryptConfirm(encryptedString: string): number {
  ...
  static encryptRole(id: number, role: string): string {
    const data = `${id}:${role}`;
    const iv = crypto.randomBytes(16);
    const key = crypto.pbkdf2Sync(this.secretKey, 'salt', 100000, 32, 'sha512');
    const cipher = crypto.createCipheriv('aes-256-cbc', key, iv);
  ...
  static decryptRole(encryptedString: string): { id: number; role: string } {
    const iv = Buffer.from(encryptedString.slice(0, 32), 'hex');
    const encryptedData = encryptedString.slice(32);
    const key = crypto.pbkdf2Sync(this.secretKey, 'salt', 100000, 32, 'sha512');
    const decipher = crypto.createDecipheriv('aes-256-cbc', key, iv);
